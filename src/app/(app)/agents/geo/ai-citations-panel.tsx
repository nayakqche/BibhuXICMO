...
// ---------------------------------------------------------------------------
// Brand icons (minimal inline SVGs so we don't pull in an icon library).
// ---------------------------------------------------------------------------
function PlatformIcon({ k, className }: { k: PlatformKey; className?: string }) {
  ...
  switch (k) {
    ...
    case "chatgpt":
      return (
        <svg viewBox="0 0 24 24" fill="currentColor" className={cn(cls, "text-zinc-100")}>
    ...
    case "grok":
      return (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={cn(cls, "text-zinc-200")}>
...
export function AiCitationsPanel({
  ...
  function refresh() {
    ...
    startTransition(async () => {
      ...
      const start = await startAiCitationsRefreshAction({ domain, country: bundle?.country ?? "us" });
      if (!start.ok) {
        setProgress(null);
        toast.error("Refresh failed", { description: start.error, duration: 8000 });
        return;
      }
      if (!("pending" in start)) {
        // Shouldn't happen for refresh, but accept cached data anyway.
        if (start.data) setBundle(start.data);
        setProgress(null);
        return;
      }
      const country = bundle?.country ?? "us";
      const startedAt = Date.now();
      while (Date.now() - startedAt < POLL_MAX_MS) {
        await new Promise((r) => setTimeout(r, POLL_INTERVAL_MS));
        let poll;
        try {
          poll = await pollAiCitationsAction({
            domain,
            country,
            runId: start.runId,
        ...
        if (!poll.ok) {
          setProgress(null);
          toast.error("Apify run failed", { description: poll.error, duration: 9000 });
          return;
        }
        if (poll.status === "DONE") {
          setBundle(poll.data);
        ...
        setProgress({ elapsedMs: Date.now() - startedAt, statusMsg: poll.statusMessage });
      }
      setProgress(null);
      toast.error("Apify run didn't finish within 3 minutes.");
    });
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        {domain && !bundle && !progress && (
          <p className="rounded-md border border-dashed py-8 text-center text-sm text-muted-foreground">
            No AI citation snapshots yet for <span className="font-medium">{domain}</span>.
            <br />
            Click <span className="font-medium">Run check</span> to scan AI Overviews,
            ChatGPT, Gemini, Perplexity, Copilot, and Grok.
          </p>
        )}

        {progress && (
          <div className="rounded-md border border-dashed bg-muted/10 px-3 py-4 text-sm">
            <div className="flex items-center gap-3">
              <Loader2 className="h-4 w-4 shrink-0 animate-spin text-primary" />
              <div>
                <div className="font-medium">Scanning AI search…</div>
                <div className="text-xs text-muted-foreground">
                  {progress.statusMsg ?? "Spinning up the Apify actor."}{" "}
                  <span className="tabular-nums">{Math.floor(progress.elapsedMs / 1000)}s</span>{" "}
                  elapsed · usually 30-90s.
                </div>
              </div>
            </div>
          </div>
...
function CitationsBody({ bundle }: { bundle: AiCitationsBundle }) {
  // Hero row = AI Overviews + ChatGPT (the two biggest)
  // Table = remaining 4 platforms
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-[10px] text-muted-foreground">
        Domain <span className="font-medium">{bundle.domain}</span> ·{" "}
        Updated {new Date(bundle.fetchedAt).toLocaleString()}
        {bundle.previousAt && (
          <> · Δ vs {new Date(bundle.previousAt).toLocaleDateString()}</>
        )}
      </div>
    </div>
  );
